...
CDBWrapper::CDBWrapper(const char* pszMode)
{
    assert(pszMode);
    activeBatch = NULL;
...
bool CDBWrapper::ReadTxIndex(uint256 hash, CTxIndex& txindex)
{
    txindex.SetNull();
    return Read(make_pair(string("tx"), hash), txindex);
}

bool CDBWrapper::UpdateTxIndex(uint256 hash, const CTxIndex& txindex)
{
    return Write(make_pair(string("tx"), hash), txindex);
}

bool CDBWrapper::AddTxIndex(const CTransaction& tx, const CDiskTxPos& pos, int nHeight)
{
    // Add to tx index
...
bool CDBWrapper::EraseTxIndex(const CTransaction& tx)
{
    uint256 hash = tx.GetHash();
...
bool CDBWrapper::ContainsTx(uint256 hash)
{
    return Exists(make_pair(string("tx"), hash));
}

bool CDBWrapper::ReadDiskTx(uint256 hash, CTransaction& tx, CTxIndex& txindex)
{
...
bool CDBWrapper::ReadDiskTx(uint256 hash, CTransaction& tx)
{
    CTxIndex txindex;
    return ReadDiskTx(hash, tx, txindex);
}

bool CDBWrapper::ReadDiskTx(COutPoint outpoint, CTransaction& tx, CTxIndex& txindex)
{
    return ReadDiskTx(outpoint.hash, tx, txindex);
}

bool CDBWrapper::ReadDiskTx(COutPoint outpoint, CTransaction& tx)
{
    CTxIndex txindex;
    return ReadDiskTx(outpoint.hash, tx, txindex);
}

bool CDBWrapper::WriteBlockIndex(const CDiskBlockIndex& blockindex)
{
    return Write(make_pair(string("blockindex"), blockindex.GetBlockHash()), blockindex);
}

bool CDBWrapper::ReadHashBestChain(uint256& hashBestChain)
{
    return Read(string("hashBestChain"), hashBestChain);
}

bool CDBWrapper::WriteHashBestChain(uint256 hashBestChain)
{
    return Write(string("hashBestChain"), hashBestChain);
}

bool CDBWrapper::ReadBestInvalidTrust(CBigNum& bnBestInvalidTrust)
{
    return Read(string("bnBestInvalidTrust"), bnBestInvalidTrust);
}

bool CDBWrapper::WriteBestInvalidTrust(CBigNum bnBestInvalidTrust)
{
...
bool CDBWrapper::LoadBlockIndex()
{
    ...
    if (!ReadHashBestChain(hashBestChain))
    {
        if (pindexGenesisBlock == NULL)
            return true;
        return error("CDBWrapper::LoadBlockIndex() : hashBestChain not loaded");
    }
    if (!mapBlockIndex.count(hashBestChain))
        return error("CDBWrapper::LoadBlockIndex() : hashBestChain not found in the block index");
