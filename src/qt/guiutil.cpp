...
{
    // Convert blitz:// to blitz:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("blitz://"))
    {
        uri.replace(0, 12, "blitz:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "Blitz.lnk";
}
...
{
    return GetAutostartDir() / "blitz.desktop";
}
