...
#if defined(NDEBUG)
# error "Blitz cannot be compiled without assertions."
#endif
...
const string strMessageMagic = "Blitz Signed Message:\n";
...
bool CTransactionPoS::ReadFromDisk(CTransaction& tx,CDBWrapper& txdb, COutPoint prevout, CTxIndex& txindexRet)
{
    tx.SetNull();
    if (!txdb.ReadTxIndex(prevout.hash, txindexRet))
        return false;
    if (!ReadFromDisk(tx, txindexRet.pos))
        return false;
...
bool CTransactionPoS::ReadFromDisk(CTransaction& tx,CDBWrapper& txdb, COutPoint prevout)
{
    CTxIndex txindex;
...
int CMerkleTx::SetMerkleBranch(const CBlock* pblock)
{
    ...
    if (pblock == NULL)
    {
        ...
        CTxIndex txindex;
        if (!CDBWrapper("r").ReadTxIndex(GetHash(), txindex))
            return 0;
...
bool CWalletTx::AcceptWalletTransaction(CDBWrapper& txdb)
{
...
{
    {
        ...
        CTxIndex txindex;
        CTransactionPoS txPoS;
        if (txPoS.ReadFromDisk(tx, txdb, COutPoint(hash, 0), txindex))
        {
            CBlock block;
...
int64_t GetProofOfStakeReward(const CBlockIndex* pindexPrev, int64_t nFees)
{
	...
	nSubsidy = (COIN * 3 / 2) / 2; // So, basically 0.5 BLITZ
...
void static InvalidChainFound(CBlockIndex* pindexNew)
{
    if (pindexNew->nChainTrust > nBestInvalidTrust)
    {
        ...
        CDBWrapper().WriteBestInvalidTrust(CBigNum(nBestInvalidTrust));
    }
...
bool CTransactionPoS::DisconnectInputs(CTransaction& tx,CDBWrapper& txdb)
{
...
bool CTransactionPoS::FetchInputs(CTransaction& tx,CDBWrapper& txdb, const map<uint256, CTxIndex>& mapTestPool,
...
int64_t CTransactionPoS::GetValueOut(CTransaction& tx) const
{
        ...
        BOOST_FOREACH(const CTxOut& txout, tx.vout)
        {
            ...
            if (!MoneyRange(txout.nValue) || !MoneyRange(nValueOut))
                throw std::runtime_error("CTransactionPoS::GetValueOut() : value out of range");
        }
        return nValueOut;
}

bool CTransactionPoS::ConnectInputs(CTransaction& tx,CDBWrapper& txdb, MapPrevTx inputs, map<uint256, CTxIndex>& mapTestPool, const CDiskTxPos& posThisTx,
        ...
        if (!tx.IsCoinStake())
        {
            ...
            /*if (!IsProtocolV3(tx.nTime)) {
                ...
                if (nTxFee < nRequiredFee)
                    return fBlock? tx.DoS(100, error("ConnectInputs() : %s not paying required fee=%s, paid=%s", tx.GetHash().ToString(), FormatMoney(nRequiredFee), FormatMoney(nTxFee))) : false;
            }*/ // TODO: NEED TO CHECK
...
bool CBlock::DisconnectBlock(CDBWrapper& txdb, CBlockIndex* pindex)
{
	CTransactionPoS txPoS;
...
bool CBlock::ConnectBlock(CDBWrapper& txdb, CBlockIndex* pindex, bool fJustCheck)
{
	CTransactionPoS txPoS;
...
bool static Reorganize(CDBWrapper& txdb, CBlockIndex* pindexNew)
{
...
bool CBlock::SetBestChainInner(CDBWrapper& txdb, CBlockIndex *pindexNew)
{
...
bool CBlock::SetBestChain(CDBWrapper& txdb, CBlockIndex* pindexNew)
{
    uint256 hash = GetHash();
...
bool CBlock::AcceptBlock()
{
    ...
    if (GetBlockTime() > FutureDrift((int64_t)vtx[0].nTime, nHeight) && experiMental == false)
...
bool static AlreadyHave(CDBWrapper& txdb, const CInv& inv)
{
    switch (inv.type)
    {
    case MSG_TX:
